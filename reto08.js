...
function organizeGifts(gifts) {
    ...
    let result = "";
    let paquetes = "";
    for (const letter of gifts) {
        if (exp.test(letter)) {
            digit += letter
        } else {
            numbers.push(parseInt(digit));
            gitfsArray.push(letter);
            digit = "";
        }
    }
    for (const item in numbers) {
        let digit = numbers[item]
        while (digit > 50) {
            paquetes = Math.floor(digit / 50);
            digit %= 50;
            result += `[${gitfsArray[item]}]`.repeat(paquetes);
        }
        while (digit > 10) {
            paquetes = Math.floor(digit / 10);
            digit %= 10;
            result += `{${gitfsArray[item]}}`.repeat(paquetes);
        }
        if (digit > 0) {
            result += "(" + gitfsArray[item].repeat(digit) + ")";
        }
    }
    return result;
}
